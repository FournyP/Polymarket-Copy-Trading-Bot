...
program
  ...
  .action(async (options) => {
    ...
    if (config.wallets_to_track.length === 0) {
      ...
      console.log('   polymarket-bot wallets add <address>\n');
      process.exit(1);
    }
    ...
    console.log(`   Min/Max Stake: $${config.min_stake} - $${config.max_stake}`);
...
program
  .command('init')
  .description('Initialize bot configuration (interactive)')
  .action(() => {
    ...
    console.log('\nNext steps:');
    console.log('1. Add wallets: polymarket-bot wallets add <address>');
    console.log('2. Configure settings: polymarket-bot config --help');
    console.log('3. Start the bot: polymarket-bot start\n');
  });
